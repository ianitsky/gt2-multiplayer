...
def main(argv):
    ...
    identity = os.path.join('patches', 'DiscIdentity.cs')
    rows = ',\n'.join(f'        ({offset}u, {length}u),  // {name}'
                      for name, offset, length, _ in table)
    with open(identity, 'w', encoding='utf-8', newline='\n') as f:
        f.write(f'''// Generated by tools/extract_overlays.py. Do not edit by hand.
//
...
{{
    public const string Built = @"{os.path.basename(disc)}";
